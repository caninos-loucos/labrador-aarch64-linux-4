...
/** lock clock to step*/
extern int mali_clk_lock_to_step;
module_param(mali_clk_lock_to_step, int, S_IRUSR | S_IWUSR | S_IWGRP | S_IRGRP | S_IROTH);
MODULE_PARM_DESC(mali_clk_lock_to_step, "Lock clock to step");
#endif
...
int mali_module_init(void)
{
	...
	MALI_DEBUG_PRINT(2, ("MALI_FAKE_PLATFORM_DEVICE mali_module_init() registering device\n"));
...
static int mali_probe(struct platform_device *pdev)
{
	...
	if (err) {
		MALI_DEBUG_PRINT(2, ("reset platform device error :%d\n", err));
	}
...
{
	/* Nothing to do */
    MALI_DEBUG_PRINT(3, ("Mali PM: Runtime idle\n"));
	return 0;
}
#endif
...
static int mali_open(struct inode *inode, struct file *filp)
{
	...
	if (mali_miscdevice.minor != iminor(inode)) {
		MALI_PRINT_ERROR(("mali_open() Minor does not match\n, mali_miscdevice.minor = 0x%x, iminor(inode) = 0x%x", mali_miscdevice.minor, iminor(inode)));
		return -ENODEV;
	}
