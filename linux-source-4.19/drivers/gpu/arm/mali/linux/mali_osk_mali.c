...
u32 _mali_osk_get_pp_cores_number(void);
...
_mali_osk_errcode_t _mali_osk_resource_initialize(void)
{
	...
	int i, pp_core_num = 0, l2_core_num = 0, pp_core_total = 8;
	struct resource *res;
	const char *compatible_name = NULL;

    pp_core_num = _mali_osk_get_pp_cores_number();

	/* We have to divide by 2, because we caculate twice for only one pp(pp_core and pp_mmu_core). */
	if (!(pp_core_num > 0) && (0 != pp_core_num % 2)) {
		MALI_DEBUG_PRINT(2, ("The value of pp core number isn't normal."));
		return _MALI_OSK_ERR_FAULT;
	}

#ifdef CONFIG_MALI_SHARED_INTERRUPTS
	res = platform_get_resource(mali_platform_device, IORESOURCE_IRQ, 0);
	if(!res) {
		MALI_DEBUG_PRINT(2, ("IRQ resource configure isn't normal."));
		return _MALI_OSK_ERR_FAULT;
	}
#endif
	if (0 == _mali_osk_get_compatible_name(&compatible_name)) {
		if (0 == strncmp(compatible_name, "arm,mali-450", strlen("arm,mali-450")) ||
		        0 == strncmp(compatible_name, "actions,s700-gpu", strlen("actions,s700-gpu"))) {
...
#ifndef CONFIG_MALI_SHARED_INTERRUPTS
...
//&& ((i <= (pp_core_num + 1) * 2) || (i >= (pp_core_total + 1) * 2))
	for (i = MALI_OSK_RESOURCE_PP_LOCATION_END; i > MALI_OSK_RESOURCE_PP_LOCATION_START + (pp_core_num * 2) -1; i--) {
		mali_osk_resource_bank[i].base = MALI_OSK_INVALID_RESOURCE_ADDRESS;
	}
	...
	for (i = 0; i < MALI_OSK_MAX_RESOURCE_NUMBER; i++) {
		if (mali_osk_resource_bank[i].base == addr) {
			if (NULL != res) {
				...
				MALI_DEBUG_PRINT(6, ("Res find Description: %s, Base address: 0x%x, irq: %d\n", res->description, res->base, res->irq));
			}
			return _MALI_OSK_ERR_OK;
		}
	}
...
uintptr_t _mali_osk_resource_base_address(void)
{
	...
	reg_res = platform_get_resource_byname(mali_platform_device, IORESOURCE_MEM, "regs");
...
u32 _mali_osk_get_pp_cores_number(void)
{
	...
	if (0 == of_property_read_u32(node, "pp_cores_number", &pp_cores)) {
		return pp_cores;
	} else {
		MALI_DEBUG_PRINT(2, ("Couldn't find pp_cores_number in device tree configuration.\n"));
	}
