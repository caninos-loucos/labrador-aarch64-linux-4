...
static void helper_timer_callback(struct timer_list *_t)
{
	_mali_osk_timer_t *tim = from_timer(tim, _t, timer);
	if ((tim != NULL) && (tim->callback != NULL)) {
...
_mali_osk_timer_t *_mali_osk_timer_init(void)
{
	...
	if (t != NULL)
	{
		t->callback = t->data = NULL;
		timer_setup(&t->timer, helper_timer_callback, 0);
	}
	return t;
}
