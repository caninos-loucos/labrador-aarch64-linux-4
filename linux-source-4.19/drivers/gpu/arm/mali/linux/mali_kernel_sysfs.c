...
static ssize_t dvfs_utilization_gpu_read(struct file *filp, char __user *ubuf, size_t cnt, loff_t *ppos)
{
    char buf[64];
    size_t r;
    u32 uval = _mali_ukk_utilization_gpu();

    r = snprintf(buf, 64, "%u\n", uval);
    return simple_read_from_buffer(ubuf, cnt, ppos, buf, r);
}

static ssize_t dvfs_target_freq_read(struct file *filp, char __user *ubuf, size_t cnt, loff_t *ppos)
{
    char buf[64];
    size_t r;
    u32 uval = _mali_ukk_dvfs_target_freq();

    r = snprintf(buf, 64, "%u\n", uval);
    return simple_read_from_buffer(ubuf, cnt, ppos, buf, r);
}

static ssize_t dvfs_cur_fps_read(struct file *filp, char __user *ubuf, size_t cnt, loff_t *ppos)
{
    char buf[64];
    size_t r;
    u32 uval = _mali_ukk_dvfs_cur_fps();
...
static const struct file_operations dvfs_utilization_gpu_fops = {
    .owner = THIS_MODULE,
    .read = dvfs_utilization_gpu_read,
};

static const struct file_operations dvfs_target_freq_fops = {
    .owner = THIS_MODULE,
    .read = dvfs_target_freq_read,
};

static const struct file_operations dvfs_cur_fps_fops = {
    .owner = THIS_MODULE,
    .read = dvfs_cur_fps_read,
};
...
int mali_sysfs_register(const char *mali_dev_name)
{
	...
	if (ERR_PTR(-ENODEV) == mali_debugfs_dir) {
		...
		if (NULL != mali_debugfs_dir) {
			...
			struct dentry *mali_dvfs_dir;
			mali_dvfs_dir = debugfs_create_dir("dvfs", mali_debugfs_dir);
			if (mali_dvfs_dir != NULL) {
				debugfs_create_file("dvfs_utilization_gpu", 0400, mali_dvfs_dir, NULL, &dvfs_utilization_gpu_fops);
				debugfs_create_file("dvfs_target_freq", 0400, mali_dvfs_dir, NULL, &dvfs_target_freq_fops);
				debugfs_create_file("dvfs_cur_fps", 0400, mali_dvfs_dir, NULL, &dvfs_cur_fps_fops);
			}
#endif
