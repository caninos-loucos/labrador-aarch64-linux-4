...
#define MALI_SHARED_MEMORY_DEFAULT_SIZE 0x3FFFFFFF/*default is 512M*/
...
static _mali_osk_errcode_t mali_parse_config_memory(void)
{
	...
	//get total ram size
	si_meminfo(&si_info);
	//totalram is the system usable pages size
	if (si_info.totalram > 0)
	{
	    mali_shared_mem_size = (si_info.totalram * PAGE_SIZE);
	    MALI_PRINT(("si_info.totalram %d, mali_shared_mem_size %#x\n", si_info.totalram, mali_shared_mem_size));
	}
#if 0
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 32))
	if (totalram_pages > 0)
	    mali_shared_mem_size = (totalram_pages * PAGE_SIZE);
	MALI_PRINT(("Mali totalram_pages %#x in Bytes, totalram_pages %d, PAGE_SIZE = %d\n", mali_shared_mem_size, totalram_pages, PAGE_SIZE));
#else
	if (num_physpages > 0)
	    mali_shared_mem_size = (num_physpages * PAGE_SIZE);
	MALI_PRINT(("Mali num_physpages %#x int Bytes, num_physpages %d, PAGE_SIZE = %d\n", mali_shared_mem_size, num_physpages, PAGE_SIZE));
#endif
#endif
