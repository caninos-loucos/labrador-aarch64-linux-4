...
#ifdef DEBUG
	u32 corrent_address = mali_hw_core_register_read(group->mmu, MALI_MMU_REGISTER_DTE_ADDR);
	u32 corrent_status = mali_mmu_get_status(group->mmu);
	MALI_DEBUG_PRINT(4, ("Executor: Mali page current detected at 0x%x from bus id %d of type %s on %s\n",
			     (void *)(uintptr_t)corrent_address,
			     (corrent_status >> 6) & 0x1F,
			     (corrent_status & 32) ? "write" : "read",
