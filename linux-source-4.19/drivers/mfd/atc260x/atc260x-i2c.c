...
static int atc260x_i2c_probe
	(struct i2c_client *i2c, const struct i2c_device_id *id_unused)
{
	...
	of_id = of_match_device(atc260x_i2c_of_match, &i2c->dev);
	
	if (!of_id)
	{
		dev_err(&i2c->dev, "could not get device data from dts.\n");
		return -ENODEV;
	}
	...
	return atc260x_core_dev_init(atc260x);
}
